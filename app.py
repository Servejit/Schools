...
def bulk_marks():
    ...
    if not students_for_class:
        ...
        st.info(
            "Make sure the student's Class and Section "
            "exactly match the selected Class."
        )
...
def print_templates():
    ...
    st.markdown(
        """
        Upload your school's A4 paper/design here.
        
        Supported files:
        **PDF, PNG, JPG, JPEG**
        
        The uploaded design will be stored separately
        for this school and can later be used as the
        background for report cards and other printed documents.
        """
    )
    ...
    with st.expander(
        ...
        st.caption(
            f"A4 size: {page_width} × {page_height} mm"
        )
    ...
    st.caption(
        f"Total templates: {len(template_data)}"
    )
    ...
    for template in template_data:
        ...
        with st.container(
            ...
            with c1:
                ...
                st.caption(
                    f"File: {file_name_value}"
                )

            with c2:

                st.write(
                    f"**A4 {orientation_text}**"
                )
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.download_button(
                            ...
                            mime=mime_type,
                            key=f"save_template_{template_id}",
                            use_container_width=True
                        )
            ...
            with b2:
                ...
                if new_active != active:
                    ...
                    if st.button(
                        "💾 Save Status",
                        key=f"save_template_status_{template_id}",
                        use_container_width=True
                    ):
